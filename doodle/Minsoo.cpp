...
void Minsoo::Draw_minsu([[maybe_unused]]Camera camera,bool camera_move)
{
	...
	else
	{
		...
		{
		case Direction::DOWN:
			doodle::draw_image(Minsoo_Down, (position.x + camera.GetPosition().x )* block_size, (position.y + camera.GetPosition().y) * block_size, block_size, block_size);
			break;
		case Direction::UP:
			doodle::draw_image(Minsoo_Up, (position.x + camera.GetPosition().x) * block_size, (position.y + camera.GetPosition().y) * block_size, block_size, block_size);
			break;
		case Direction::RIGHT:
			doodle::draw_image(Minsoo_right, (position.x + camera.GetPosition().x) * block_size, (position.y + camera.GetPosition().y) * block_size, block_size, block_size);
			break;
		case Direction::LEFT:
			doodle::draw_image(Minsoo_left, (position.x + camera.GetPosition().x) * block_size, (position.y + camera.GetPosition().y) * block_size, block_size, block_size);
			break;
		}
